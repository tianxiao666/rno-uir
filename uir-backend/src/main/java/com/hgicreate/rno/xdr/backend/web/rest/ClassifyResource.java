...
public class ClassifyResource {
    ...
    @GetMapping("/searchClassifySample")
    public ResponseEntity<List<String>> getTestSampleType(){
        List<Record> recordList= recordRepository.findByFileTypeAndState("干扰数据","入库成功");
    ...
    public ResponseEntity<List<Task>> searchTest(@RequestParam Long beginTime,
        ...
        Page<Task> taskPage = trainService.findByNameAndStatusAndTime(name,status,
            ...
            dateTo.toInstant().atZone(ZoneId.systemDefault()).toLocalDateTime(),"分类",pageable);
    ...
    public String addClassifyTask(@RequestParam String sampleName,
        ...
        if(recordList.size() > 0){
            ...
            return "待分类数据不存在，请选择其他数据";
        }
        taskRepository.save(task);
        List<Interfere> interferes=interfereRepository.findByFileId(task.getFileId().toString());
        ...
        log.debug("分类任务待传参数为：data = {}",Arrays.deepToString(prbArr));
        //return "添加分类成功";
    ...
    @GetMapping("/downloadClassifyResult")
    @Timed
    public void downloadClassifyResult(@RequestParam Long taskId, HttpServletResponse response) throws IllegalAccessException, NoSuchFieldException, JSONException {
        Long fileId =taskRepository.findOne(taskId).getFileId();
        List<Interfere> interferes = interfereRepository.findByFileId(fileId.toString());
        String fileName = "分类任务提取结果.xlsx";
        try {
    ...
    private String transferEngToChinese(String eng){
        ...
        map.put("rb4","4级RB数");
        map.put("rb5","5级RB数");
        map.put("interfereCoefficient","干扰系数");
