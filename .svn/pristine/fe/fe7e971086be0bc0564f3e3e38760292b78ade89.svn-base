...
@Component({
  selector: 'app-area-data-t',
  templateUrl: './area-data-t.component.html',
  styleUrls: ['./area-data-t.component.css']
})
export class AreaDataCnComponent implements OnInit {
  private areaInfo: AreaInfo;
  private _code : string = '440102';
  ...
  @Output() areaSet: EventEmitter<string[]> = new EventEmitter<string[]>();
  ...
  public ChangeSel(idx: number): void{
    ...
    if (idx >= 2){
      ...
      if (item){
        ...
        if(this.bInit) {
          if('全区'===this.araes[2]) {
            arealist.push(this.codes[0]);
            arealist.push(this.codes[1]);
          }else {
            arealist.push(this.codes[0]);
            arealist.push(this.codes[1]);
            arealist.push(this.codes[2]);
          }
          this.areaSet.emit(arealist);
        }
