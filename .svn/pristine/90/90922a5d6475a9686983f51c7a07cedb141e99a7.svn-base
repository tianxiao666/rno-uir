...
@Injectable()
export class SampleClassifyService {
  env = environment;
  ...
  public getData(inputs: FinalSearchValue): Promise<any> {
    ...
    const url = `${this.env.host}/api/searchClassify?beginTime=${beginMillis}` +
      `&endTime=${endMillis + 24 * 60 * 60 * 1000}&name=${inputs.classifyName}&status=${inputs.classifyStatus}&city=${inputs.city}` +
